$(document).ready(function() {
    ...
    $.ajax({
        ...
        beforeSend: function() {
            $('table.list-backup-locations tbody').addClass('spinner-grow').empty();            
        },
        success: function(response) {
            
            $('table.list-backup-locations tbody').removeClass();

            for(var bl in response.backupStorageLocations){
                $('table.list-backup-locations tbody').append('<tr><td><strong>' + response.backupStorageLocations[bl].metadata.name + '</strong></td>' +
